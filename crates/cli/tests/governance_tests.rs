...
use clap::Parser;
use protocore_cli::commands::governance::{ProposalType, VoteArgs, VoteOption};

#[test]
fn test_proposal_type_display() {
    assert_eq!(
        format!("{}", ProposalType::ParameterChange),
        "Parameter Change"
    );
    assert_eq!(
        format!("{}", ProposalType::SoftwareUpgrade),
        "Software Upgrade"
    );
    assert_eq!(
        format!("{}", ProposalType::CommunitySpend),
        "Community Spend"
    );
